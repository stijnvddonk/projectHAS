...
//			if (this.debug) logging.log("Call next class\n");
	...
	public boolean validateUser(String user, String saltedPass, String pass) {
		...
		if (user.isEmpty() || pass.isEmpty()) {
			if (this.debug)
				DataLogger.log("Incorrect not all field filled in.\n");
		} else if (this.active != 1) {
			if (this.debug)
				DataLogger.log("User is deactivated.\n");
		} else if (!user.equals(this.username) || !saltedPass.equals(this.password)) {
			if (this.debug)
				DataLogger.log("Incorrect username or password.\n");
			...
			if (this.debug)
				DataLogger.log("Correct credentials.\n");
		}
